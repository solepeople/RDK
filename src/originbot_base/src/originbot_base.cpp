...
{
    // 加载参数
    std::string port_name="ttyUSB";    
    this->declare_parameter("port_name");           //声明及获取串口号参数
    this->get_parameter_or<std::string>("port_name", port_name, "ttyUSB1");
    ...
    try
    {
        ...
        serial_.setBaudrate(230400);                                     //设置波特率
...
void OriginbotBase::fileRecord(DataFrame &frame)
{
    //打开文件
    FILE *outFile = fopen("/root/rdkbot/src/data/data.txt", "a");
    uint8_t arr[11] = {frame.header, frame.id, frame.length, frame.data[0], frame.data[1], frame.data[2], 
                        frame.data[3], frame.data[4], frame.data[5], frame.check, frame.tail};
    int i;
    for(i = 0; i < 11; i++)
    {
        //写入数据
        fprintf(outFile, "%02x, ", arr[i]);
    }
    fprintf(outFile, "\n");
    //关闭文件
    fclose(outFile);
...
void OriginbotBase::IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
    float norm;
    float vx, vy, vz;
    float ex, ey, ez;
...
void OriginbotBase::processAccelerationData(DataFrame &frame)
{
    ...
    imu_data_.acceleration_x = imu_conversion(frame.data[1], frame.data[0]) / 32768 * 4 * 9.8;
    imu_data_.acceleration_y = imu_conversion(frame.data[3], frame.data[2]) / 32768 * 4 * 9.8;
    imu_data_.acceleration_z = imu_conversion(frame.data[5], frame.data[4]) / 32768 * 4 * 9.8;
}
...
void OriginbotBase::processAngularData(DataFrame &frame)
{
    ...
    if(use_imu_)
    {
        IMUupdate(imu_data_.angular_x*1.57079,imu_data_.angular_y*1.57079,imu_data_.angular_z*1.57079,imu_data_.acceleration_x,imu_data_.acceleration_y,imu_data_.acceleration_z);
        imu_publisher();        
    }
...
void sigintHandler(int sig)
{
    ...
    serial.setPort("/dev/ttyUSB0");                                   //选择要开启的串口号
...
{
    //清理记录文件
    FILE *file = fopen("/root/rdkbot/src/data/data.txt", "w");
    fclose(file);
    // 初始化ROS节点
